...
pub fn mean_sidereal_time(julian_day: f64) -> f64 {
    let days_since_j2000 = julian_day - 2_451_545.0_f64;
    let jc = days_since_j2000 / 36_525.0_f64;
...
mod tests {
    ...
    fn mean_sidereal_time_matches_reference_implementation() {
        ...
        assert!((nu0 - 318.515_578).abs() < 1e-4_f64);
    }
    ...
    fn mean_sidereal_time_higher_order_terms_at_jc_100() {
        ...
        assert!((higher_order - 3.853_496_881_94).abs() < 5e-6_f64);
    }
    ...
    fn mean_sidereal_time_wraps_into_zero_360() {
        for &jd in &[
            ...
            0.0_f64,
            2_451_545.0_f64,
            2_452_930.312_847_f64,
            5_000_000.0_f64,
        ] {
            let nu0 = mean_sidereal_time(jd);
            assert!((0.0_f64..360.0_f64).contains(&nu0));
        }
    }
    ...
    fn apparent_sidereal_time_shifts_linearly_with_delta_psi() {
        ...
        for &dp in &[-1e-2_f64, -1e-4_f64, 1e-6_f64, 5e-3_f64] {
            let shifted = apparent_sidereal_time(nu0, dp, epsilon);
            assert!(dp.mul_add(-cos_eps, shifted - baseline).abs() < 1e-13_f64);
        }
    }

    #[test]
    fn apparent_sidereal_time_correction_vanishes_at_quarter_turn_obliquity() {
        for &dp in &[-1.0_f64, 0.0_f64, 1.0_f64] {
            let nu = apparent_sidereal_time(217.5, dp, 90.0);
            assert!((nu - 217.5).abs() < 1e-13_f64);
        }
    }
}
